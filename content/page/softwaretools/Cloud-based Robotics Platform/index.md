...
1. SSH to the server:
    - Option 1: Terminal: `ssh USERNAME@SERVER-IP`
    - Option 2: [vscode-remote-explorer](https://code.visualstudio.com/docs/remote/ssh)
...
1. Connect to the remote desktop
    - For safety, do not expose port 3389 to the public network interface. Make it available on `localhost` and forward the port to your local PC via [vscode-remote-explorer](https://code.visualstudio.com/docs/remote/ssh) or the `SSH` command `ssh -L 3389:localhost:3389 USERNAME@SERVER-IP`
...
1. Mount the VM directory on the host system, so we can edit files with VScode. **On the host machine terminal**:
    - Install `sshfs`: `sudo apt install sshfs`
    - Create a mount point: `mkdir ~/vm-binderhub`
    - Mount the directory (change it to your VM IP): `sshfs binderhub@192.168.122.247:/home/binderhub ~/vm-binderhub`
    - `cd ~/vm-binderhub`
    - Create a config file `binder.yaml`.
    - Modify the config file `binder.yaml` with VScode:
        ```yaml
        registry:
          username: <docker-id>
          password: <password>
    ...
    - Above is the minimal config of the deployment. This needs to be configured according to the resources of the specific server and network environment, you can refer to [binder.intel4coro.de-deploy](https://github.com/IntEL4CoRo/binder.intel4coro.de-deploy/blob/main/binder.config) and [mybinder.org-deploy](https://github.com/jupyterhub/mybinder.org-deploy/blob/main/mybinder/values.yaml) for advanced configuration.
...
The external IPs in the previous step are just the IPs of the KVM virtual network, and these IPs are only accessed by services within the server local network. To make the service accessible from the public network, we need to set up a proxy server to listen for requests from the public network.
...
- Forward port `10086` to the client PC, or use the server remote desktop to open Firefox.
...
- Add `Proxy Host` for the domain names.
    - First, check if the DNS records are active: `ping <your domain name>`, see if it echoes the server public IP.
    - **Log in to the VM**, Check the k8s service again: `kubectl get svc -A`
    - Proxy domain name `<domain name for BinderHub>` to the IP of `binder` (192.168.122.102:80), and add custom config:
        ```
        location /robots.txt {
    ...
    - Proxy domain name `<domain name for JupyterHub>` to the IP of `proxy-public` (192.168.122.101:80), enable cache, websocket and add custom config:
        ```
